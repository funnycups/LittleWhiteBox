...
function bindMessageHandlers() {
    ...
    messageHandlers.messageReceived = handleAIMessage;
    messageHandlers.messageSwiped = handleAIMessage;
    messageHandlers.messageEdited = handleAIMessage;
    messageHandlers.messageUpdated = handleAIMessage;

    eventSource.on(event_types.MESSAGE_RECEIVED, messageHandlers.messageReceived);
    if (event_types.MESSAGE_SWIPED) {
        eventSource.on(event_types.MESSAGE_SWIPED, messageHandlers.messageSwiped);
    }
    if (event_types.MESSAGE_EDITED) {
        eventSource.on(event_types.MESSAGE_EDITED, messageHandlers.messageEdited);
    }
    if (event_types.MESSAGE_UPDATED) {
        eventSource.on(event_types.MESSAGE_UPDATED, messageHandlers.messageUpdated);
    }
}

function unbindMessageHandlers() {
    if (messageHandlers.messageReceived) {
        eventSource.removeListener(event_types.MESSAGE_RECEIVED, messageHandlers.messageReceived);
    }
    if (messageHandlers.messageSwiped && event_types.MESSAGE_SWIPED) {
        eventSource.removeListener(event_types.MESSAGE_SWIPED, messageHandlers.messageSwiped);
    }
    if (messageHandlers.messageEdited && event_types.MESSAGE_EDITED) {
        eventSource.removeListener(event_types.MESSAGE_EDITED, messageHandlers.messageEdited);
    }
    if (messageHandlers.messageUpdated && event_types.MESSAGE_UPDATED) {
        eventSource.removeListener(event_types.MESSAGE_UPDATED, messageHandlers.messageUpdated);
    }

// 轻量版：只处理新增/可见消息，弱引用缓存，IO + MO 节流（含优化） 
...
const observeVisibility = (nodes) => {
    ...
    io = new IntersectionObserver((entries) => {
      for (const e of entries) {
        ...
        if (e.isIntersecting) {
          ...
          io.unobserve(e.target); // 处理一次即停止观察，省资源
        }
      }
    }, {
  ...
  const maxDistancePx = 3000; // 只关注距离视口上下 3000px 内
...
// rAF 批处理 MutationObserver 的新增，避免抖动
...
const registerButtonToSubContainer = (messageId, buttonEl) => {
  ...
  processOneMessage(message); // 确保有折叠按钮
